...
public:
  ...
  DrawFitResult(std::function<double(const double *, const double *)> background,
                std::function<double(const double *, const double *)> signal,
                std::function<double(const double *, const double *)> total, TGraphErrors *graph,
                int NSignal, int SigTyp);

  void DrawResult(std::vector<double> resPara, double x_first, double x_last, double fit_x_low,
                  double fit_x_high);
  // for single graph without bin
  void DrawResultSingle(/*std::vector<double> resPara, */ double x_first, double x_last,
                        double fit_x_low, double fit_x_high);

  void DrawBGResult(std::vector<double> *resPara, double x_first, double x_last, double fit_x_low,
                    double fit_x_high);
  // for every single bin
  void DrawBGSGResult(std::vector<double> resPara, double x_first, double x_last, double fit_x_low,
                      double fit_x_high);
  void DrawSGResult(std::vector<double> *resPara, double x_first, double x_last, double fit_x_low,
                    double fit_x_high);
