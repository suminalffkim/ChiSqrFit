...
void RealData::GetDataSG() {
  ...
  for (int bin = 0; bin < Hpipluspiminuseta_m->GetNbinsX(); bin++) {
    _xSingle.push_back(Hpipluspiminuseta_m.GetBinCenter(bin));
    _ySingle.push_back(Hpipluspiminuseta_m.GetBinContent(bin));
    _delta_ySingle.push_back(std::sqrt(Hpipluspiminuseta_m.GetBinContent(bin)));
    _delta_xSingle.push_back(0);
  }

  // Hpiminuspiminuseta_m->Rebin(2);
  std::vector<double> delta_x2;
  for (int bin = 0; bin < Hpiminuspiminuseta_m->GetNbinsX(); bin++) {
    _xPiMiPiMiEtaSingle.push_back(Hpiminuspiminuseta_m->GetBinCenter(bin));
    _yPiMiPiMiEtaSingle.push_back(Hpiminuspiminuseta_m->GetBinContent(bin));
    _delta_y_PiMiPiMiEtaSingle.push_back(std::sqrt(Hpiminuspiminuseta_m->GetBinContent(bin)));
    delta_x2Single.push_back(0);
  }

  std::cout << "We just stored the data we need for Chi Square without binning.." << std::endl;
  TGraphErrors *Graph = new TGraphErrors(_xSingle.size(), &_xSingle.front(), &_ySingle.front(),
                                         &_delta_xSingle.front(), &_delta_ySingle.front());
  GDataSingle.push_back(Graph);
